...
OUTPUT_FILENAME = DERIVED_MANUAL_INSPECTION_DIR / "am_samples.csv"
SAME_ENTRIES_FILE = DERIVED_CLASSIFICATION_DIR / "inputs" / "final_am_classification_3.csv"
...
def proportional_sampler_exact_match():
    """
    1. Loads manual_classification_am.csv files
    2. Calculates proportional samples needed (332 total - manual entries count)
    3. Samples files proportionally based on dataset sizes
    4. Applies English language filter to ALL files (manual entries + new samples)
    ...
    # --- Phase 0: Load manual_classification_am.csv ---
    ...
    if os.path.exists(SAME_ENTRIES_FILE):
        try:
            ...
            print(f"✅ Loaded {same_entries_count} entries from manual_classification_am.csv")
            ...
            print(f"❌ Error loading manual_classification_am.csv: {e}")
            same_entries_df = None
    else:
        print(f"⚠️  File not found: {SAME_ENTRIES_FILE}")
        print("   Proceeding without manual classification entries...")
    ...
    print(f"   - From manual_classification_am: {same_entries_count} files")
    print(f"   - Need to sample: {remaining_needed} files")
    
    if remaining_needed <= 0:
        print(f"\n⚠️  manual_classification_am already has {same_entries_count} files (>= {TOTAL_SAMPLE_SIZE})")
        print("   Will use only manual classification entries and apply language filter...")
        remaining_needed = 0
    ...
    for agent in AGENT_NAMES:
        filename = get_manifest_dataset(agent).original_path
